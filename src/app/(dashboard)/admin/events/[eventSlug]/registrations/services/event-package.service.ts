...
export async function updateEventPackage(
  ...
  const response = await fetch(`${apiUrl}/admin/events/${eventId}/packages/${packageId}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      ...getAuthHeaders()
...
export async function deactivateEventPackage(eventSlug: string, packageId: string): Promise<void> {
  const eventId = await getEventIdBySlug(eventSlug)

  const response = await fetch(`${apiUrl}/admin/events/${eventId}/packages/${packageId}/deactivate`, {
    method: 'POST',
    headers: getAuthHeaders()
  })
