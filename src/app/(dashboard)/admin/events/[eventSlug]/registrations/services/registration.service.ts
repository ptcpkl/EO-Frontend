...
const normalizeStatus = (value: string): Registration['status'] => {
  ...
  if (status === 'CANCELLED' || status === 'CANCELED' || status === 'FAILED' || status === 'EXPIRED') return 'CANCELLED'
  if (status === 'PENDING' || status === 'PENDINGPAYMENT') return 'PENDING'

  // Registered is the canonical paid/active registration status.
  // Keep legacy Paid compatible with older API/data during rollout.
  if (status === 'REGISTERED' || status === 'PAID') return 'REGISTERED'
...
export async function getRegistrations(eventSlug: string, filters?: RegistrationFilters): Promise<Registration[]> {
  ...
  if (response.status === 403) throw new Error('You are not authorized to view registrations.')
