  ...
  const updateSession = async (session: QuizSessionResponse, action: 'open' | 'cancel') => {
    try {
      ...
      setNotice(action === 'open'
        ? `${updated.name} is now open. Participants can join with ${updated.roomCode}.`
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        ...
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, justifyContent: 'space-between', gap: 2.5 }}>
          <Box>
            ...
            <Typography color='text.secondary' sx={{ mt: 1, maxWidth: 800 }}>
              Create independent rooms, allocate unique Question Bank items, then share the room code or QR with participants.
            </Typography>
          </Box>
          <Button
            component={NextLink}
      ...
      {!quiz ? (
        <Alert
          severity='info'
          action={
            <Button component={NextLink} href={`/admin/events/${encodeURIComponent(event.id)}/edit`}>
              Add Quiz
            </Button>
          }
        >
        ...
        <>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' }, gap: 2.5 }}>
            <Card variant='outlined'>
              <CardContent>
                ...
                <Typography variant='h4' fontWeight={750} sx={{ mt: 1 }}>{activeQuestionCount}</Typography>
                <Typography variant='caption' color='text.secondary'>active questions</Typography>
              </CardContent>
            </Card>
            <Card variant='outlined'>
              <CardContent>
                <Typography variant='body2' color='text.secondary'>Unused estimate</Typography>
                <Typography variant='h4' fontWeight={750} sx={{ mt: 1 }}>{estimatedAvailable}</Typography>
                <Typography variant='caption' color='text.secondary'>backend enforces exact allocation</Typography>
              </CardContent>
            </Card>
            <Card variant='outlined'>
              <CardContent>
                <Typography variant='body2' color='text.secondary'>Rooms</Typography>
                <Typography variant='h4' fontWeight={750} sx={{ mt: 1 }}>{sessions.length}</Typography>
                <Typography variant='caption' color='text.secondary'>created sessions</Typography>
              </CardContent>
            </Card>
          </Box>
            ...
            <CardContent sx={{ p: { xs: 3, md: 4 }, display: 'grid', gap: 3 }}>
              <Box>
                ...
                <Typography variant='body2' color='text.secondary' sx={{ mt: 0.75 }}>
                  Questions are allocated when the room is created. Each Question Bank item can belong to only one room.
                </Typography>
              </Box>
          ...
          <Box>
            ...
            <Typography variant='body2' color='text.secondary' sx={{ mt: 0.75 }}>
              Open a room only when you are ready for participants to join.
            </Typography>
          </Box>
          ...
          {sessions.length === 0 ? (
            <Card variant='outlined'>
              <CardContent sx={{ py: 7, textAlign: 'center' }}>
                <i className='tabler-device-gamepad-2 text-4xl' />
                <Typography variant='h6' sx={{ mt: 2 }}>No Quiz rooms yet</Typography>
                <Typography variant='body2' color='text.secondary' sx={{ mt: 0.75 }}>
                  Create the first session above.
                </Typography>
              </CardContent>
            </Card>
          ) : (
                  ...
                  <CardContent sx={{ display: 'grid', gap: 3 }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2, alignItems: 'flex-start' }}>
                      <Box>
                        ...
                        <Typography variant='body2' color='text.secondary' sx={{ mt: 0.5 }}>
                          {session.questionCount} questions • {session.questionDurationSeconds}s each
                        </Typography>
                      </Box>
                      <Chip label={session.status} color={statusColor[session.status]} variant='tonal' size='small' />
                    </Box>

                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '170px minmax(0, 1fr)' }, gap: 3, alignItems: 'center' }}>
                      <Box
                        component='img'
                        src={getQuizRoomQrUrl(session.roomCode)}
                        alt={`QR code for ${session.roomCode}`}
                        sx={{ width: 170, height: 170, p: 1, bgcolor: 'common.white', borderRadius: 2, border: theme => `1px solid ${theme.palette.divider}` }}
                      />

                      <Box sx={{ display: 'grid', gap: 2 }}>
                        <Box>
                          <Typography variant='caption' color='text.secondary'>ROOM CODE</Typography>
                          <Typography variant='h4' fontWeight={800} letterSpacing={3}>{session.roomCode}</Typography>
                        </Box>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2, alignItems: 'center' }}>
                      <Typography variant='body2' color='text.secondary'>
                        <strong>{session.participantCount}</strong> joined • {session.remainingCapacity} of {session.capacity} spots left
                      </Typography>
                      <Box sx={{ display: 'flex', gap: 1.25 }}>
                        {session.status === 'Draft' && (
                          <Button
                            ...
                            startIcon={<i className='tabler-door-enter' />}
                            onClick={() => void updateSession(session, 'open')}
                          >
                            Open Room
                          </Button>
                        )}
                        {(session.status === 'Draft' || session.status === 'Open') && (
                          <Button
                            ...
                            color='error'
                            disabled={actionId === session.id}
                            onClick={() => void updateSession(session, 'cancel')}
                          >
                            Cancel
                          </Button>
                        )}
